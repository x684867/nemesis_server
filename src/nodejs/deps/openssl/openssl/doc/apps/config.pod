...
contain specific package configuration information. The B<name> represents
the name of the I<configuration package> the meaning of the B<value> is 
package specific: it may, for example, represent a further configuration
section containing configuration package specific information. E.g.
...
Currently there are two configuration packages. One for ASN1 objects another
for ENGINE configuration.

=head2 ASN1 OBJECT CONFIGURATION package

This package has the name B<oid_section>. The value of this variable points
...
functionality not all do. By using the ASN1 OBJECT configuration package
...
=head2 ENGINE CONFIGURATION package

This ENGINE configuration package has the name B<engines>. The value of this
