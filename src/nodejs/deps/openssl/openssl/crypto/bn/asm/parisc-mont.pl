...
# On PA-7100LC this package performs ~90-50% better, less for longer
...
# package picks halves of 64-bit values in reverse order and pretends
...
# one.
#
# In case it wasn't clear. The package has two distinct code paths:
# PA-RISC 1.1 and PA-RISC 2.0 ones. Latter features carry-free 64-bit
# additions and 64-bit integer loads, not to mention specific
# instruction scheduling. In 64-bit build naturally only 2.0 code path
# is assembled. In 32-bit application context both code paths are
# assembled, PA-RISC 2.0 CPU is detected at run-time and proper path
# is taken automatically. Also, in 32-bit build the package imposes
