...
function Packer (props) {
  ...
  // only do the magic bundling stuff for the node_packages folder that
  // lives right next to a package.json file.
  this.bundleMagic = this.parent &&
                     this.parent.packageRoot &&
                     this.basename === "node_packages"

  // in a node_packages folder, resolve symbolic links to
...
Packer.prototype.readBundledLinks = function () {
  ...
  this.pause()
  fs.readdir(this.path + "/node_packages", function (er, list) {
    ...
    list.forEach(function (pkg) {
      ...
      var pd = this.path + "/node_packages/" + pkg
...
Packer.prototype.applyIgnores = function (entry, partial, entryObj) {
  ...
  if (entry === "node_packages" && this.packageRoot) return true
  ...
  // in a node_packages folder, we only include bundled dependencies
  // also, prevent packages in node_packages from being affected
...
Packer.prototype.getChildProps = function (stat) {
  ...
  // only follow symbolic links directly in the node_packages folder.
  props.follow = false
  return props
}
...
Packer.prototype.sort = function (a, b) {
  ...
  // deps go in the back
  if (a === "node_packages") return 1
  if (b === "node_packages") return -1
