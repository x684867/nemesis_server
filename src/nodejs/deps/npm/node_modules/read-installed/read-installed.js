...
/*
This will traverse through all node_packages folders,
...
, dependencies:
  { foo :
    { version: "1.2.3"
    , path: "/path/to/node_packages/foo"
    , parent: <Circular: root>
    , dependencies:
      { bar:
        { parent: <Circular: foo>
        , path: "/path/to/node_packages/foo/node_packages/bar"
        , version: "2.3.4"
...
for each folder in installed
  obj.dependencies[folder] = READ(packagefolder+node_packages+folder,
...
as far as the left-most node_packages folder.
2. Ignore anything in node_packages that isn't a package folder.
...
function readInstalled_ (folder, parent, name, reqver, depth, maxDepth, cb) {
  ...
  fs.readdir(path.resolve(folder, "node_packages"), function (er, i) {
  ...
  function next (er) {
    ...
    asyncMap(installed, function (pkg, cb) {
      ...
      if (depth >= maxDepth) {
        ...
        var pkgfolder = path.resolve(folder, "node_packages", pkg)
      ...
      readInstalled_( path.resolve(folder, "node_packages/"+pkg)
