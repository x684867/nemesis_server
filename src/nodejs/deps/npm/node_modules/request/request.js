...
Request.prototype.init = function (options) {
  ...
  var protocol = self.proxy && !self.tunnel ? self.proxy.protocol : self.uri.protocol
    , defaultpackages = {'http:':http, 'https:':https}
    , httppackages = self.httppackages || {}
    ;
  self.httppackage = httppackages[protocol] || defaultpackages[protocol]

  if (!self.httppackage) return this.emit('error', new Error("Invalid protocol"))
  ...
  if (!self.agent) {
    ...
    if (options.agentClass) {
      ...
      self.agentClass = self.httppackage.Agent
    }
  }
...
// httppackage, Tunneling agent, and/or Forever Agent in use.
...
Request.prototype.getAgent = function () {
  ...
  if (Agent !== this.httppackage.Agent) {
    poolKey += Agent.name
  }

  if (!this.httppackage.globalAgent) {
    // node 0.4.x
  ...
  if (this.pool === globalPool && !poolKey && Object.keys(options).length === 0 && this.httppackage.globalAgent) {
    // not doing anything special.  Use the globalAgent
    return this.httppackage.globalAgent
  }
...
Request.prototype.start = function () {
  ...
  self.req = self.httppackage.request(reqOptions, self.onResponse.bind(self))
