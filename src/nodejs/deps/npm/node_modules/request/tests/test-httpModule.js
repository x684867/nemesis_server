...
function wrap_request(name, package) {
  // Just like the http or https package, but note when a request is made.
  var wrapped = {}
...
function run_tests(httppackages) {
  var to_https = 'http://localhost:'+plain_server.port+'/to_https'
  var to_plain = 'https://localhost:'+https_server.port+'/to_plain'

  request(to_https, {'httppackages':httppackages, strictSSL:false}, function (er, res, body) {
    if (er) throw er
    assert.equal(body, 'https', 'Received HTTPS server body')
    done()
  })

  request(to_plain, {'httppackages':httppackages, strictSSL:false}, function (er, res, body) {
    if (er) throw er
...
function done() {
  ...
  if(passed == expected) {
    ...
    assert.equal(faux_requests_made.http, 4, 'Wrapped http package called appropriately')
    assert.equal(faux_requests_made.https, 4, 'Wrapped https package called appropriately')
