...
PromZard.prototype.loaded = function () {
  ...
  var mod = this.ctx.package = this.makepackage()
  this.ctx.require = function (path) {
    return mod.require(path)
  }
  this.ctx.require.resolve = function(path) {
    return package._resolveFilename(path, mod);
  }
...
PromZard.prototype.makepackage = function () {
  var mod = new package(this.file, package)
  mod.loaded = true
  mod.filename = this.file
  mod.id = this.file
  mod.paths = package._nodepackagePaths(path.dirname(this.file))
  return mod
}
