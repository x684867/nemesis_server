...
A prompting wizard for building files from specialized PromZard packages.
Used by `npm init`.
...
The inputFile is just a node package.  You can require() things, set
package.exports, etc.  Whatever that package exports is the result, and it
...
to the package (you can get this via Node's `require.resolve`.)  Also,
...
package.  You can get quite fancy with this, passing in existing configs
and so on.
...
In the promzard input package, you can call the `prompt` function.
...
If there are any functions on the promzard input package's exports, then
promzard will call each of them with a callback.  This way, your package
...
For example, this works fine in a promzard package:
