...
In fact, the Streams2 API was developed while using it for packages in
the npm registry.  At the time of this writing, [37 published Node
packages](https://npmjs.org/browse/depended/readable-stream) already
are using the
...
The `domain` package has been elevated from "Experimental" to
...
happening in userland packages.  Now, we've finally taken that to its
logical conclusion, and moved our iteration on **core** packages into
...
The userland package can live on as a compatibility layer so that
...
npm would not be what it is without the registry of published packages.
...
[tlsnappy](https://github.com/indutny/tlsnappy) package, which may make
