...
function loggerClass(loggerSourceString,loggerProcessPID,loggerPriorityString,loggerFacilityString){
	...
	this.log=function(message){
		...
		var m=screen.now()+
			  ":"+this.source+"("+this.pid+")"+
			  "["+this.facility+"]"+
			  "["+this.priority+"]"+
			  "("+message.length+")"+
			  message;

		if(config.logger.screen.minDisplayPriority <= this.priority) screen.write(m);
		id(config.logger.syslog.minDisplayPriority <= this.priority) syslog.write(m);
	}
}